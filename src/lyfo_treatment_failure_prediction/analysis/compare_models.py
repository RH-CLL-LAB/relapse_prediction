...
Example usage (library-style):
    ...
    import pandas as pd
    from lyfo_treatment_failure_prediction.analysis.compare_models import bootstrap_compare
...
class BootstrapResult:
    ...
    levels: pd.DataFrame  # per-model metric levels with CIs
...
def _bootstrap_pvalue_two_sided(deltas: np.ndarray) -> float:
    """Two-sided bootstrap p-value: 2 * min(Pr(Δ<=0), Pr(Δ>=0)) with
...
def bootstrap_compare(
    ...
    BootstrapResult with two tables:
      ...
      - levels: per-model per-metric levels with CIs
    """
    ...
    # Storage
    level_samples: Dict[str, Dict[str, List[float]]] = {
        m: {k: [] for k in metrics} for m in model_names
    }
    delta_samples: Dict[Tuple[str, str], Dict[str, List[float]]] = {
        pair: {k: [] for k in metrics} for pair in pairs
    }
    ...
    ci_low_col = f"delta_ci_low_{alpha_pct}"
    ci_high_col = f"delta_ci_high_{alpha_pct}"

    delta_df["star"] = [
        star_by_ci(row[ci_low_col], row[ci_high_col])
    ...
    if "p_adj" in delta_df.columns:
        cols.append("p_adj")
    cols += ["star", "stars_p"]

    delta_df = (
        delta_df[cols].sort_values(["metric", "pair"]).reset_index(drop=True)
    )
...
read_files = [pd.read_csv(f"data/{x}") for x in test_specific_file_names]

only_relevant_columns = []
for df in read_files:
    colnames = [
        c
        for c in df.columns
        if "patientid" in c
...
# Main outcome: successful treatment (2 years)
results = bootstrap_compare(
...
# ---------------------------------------------------------------------------
# Prep for other outcomes (individual_outcomes)
# ---------------------------------------------------------------------------
...
read_files = [
    pd.read_csv(f"data/individual_outcomes/{x}") for x in test_specific_file_names
]
...
for df in only_relevant_columns:
    data = pd.merge(data, df)
