...
class FeatureMaker:
    ...
    def add_features_given_ratio(
        ...
        prediction_column: str = "date_treatment_1st_line",
        proportion: int = 0.2,
        ...
        """
        super_subset = self.long_data[
            self.long_data["data_source"] == data_source
        ].reset_index(drop=True)

        # merge with prediction time to get lookback window specific timings

        super_subset = super_subset.merge(self.wide_data[[self.entity_id_col_name, prediction_column]])

        # make lookback specific rare feature names to exclude from other timewindows

        rare_features = [f"rare_{lookback.days}" for lookback in lookbacks]
        dictionary_of_features = {}
        for lookback in lookbacks:
            super_subset["lookback"] = super_subset[prediction_column] - lookback
            subset = super_subset[super_subset[self.timestamp_col_name] >= super_subset["lookback"]]
            ...
            # delete rares from subsequent feature creations
            subset_codes = (
                subset.groupby([self.entity_id_col_name, variable_col])
                .count()
                .reset_index()
            )
            subset_codes = subset_codes[~subset_codes[variable_col].isin(rare_features)].reset_index(drop = True)

            values = subset_codes[variable_col].value_counts().reset_index()
            
            
            n_values = len(values)
            values["count"] = values["count"] / len(self.wide_data)
            values = values[values["count"] > proportion]
            values = values[variable_col].values

            for value in values:
                if dictionary_of_features.get(value):
                    dictionary_of_features[value].append(lookback)
                else:
                    dictionary_of_features[value] = [lookback]

            if collapse_rare_conditions_to_feature and n_values != len(values):
                
                # have to remake logic to have append to the dataframe instead

                days_of_lookback = lookback.days

                rare_values = subset[~subset[variable_col].isin(values)].reset_index(drop = True)
                
                rare_values = rare_values[[self.entity_id_col_name, self.timestamp_col_name, variable_col, value_column, "data_source"]].reset_index(drop = True)

                rare_values.loc[:, variable_col] = f"rare_{days_of_lookback}"
                
                self.long_data = pd.concat([self.long_data, rare_values]).reset_index(drop = True)
                ...
                specification = {
                    ...
                    "variable_value": f"rare_{days_of_lookback}",
                    "agg_function": [
                        CountAggregator()
        ...
        for value, lookback_list in dictionary_of_features.items():
            specification = {
                "data_source": f"{data_source}",
