...
class FeatureMaker:
    def __init__(
        ...
        feature_processing_order: str = "alphabetical",
        ...
        self.feature_processing_order = feature_processing_order
    ...
    def add_features_given_ratio(
        ...
        n_values = len(values)
        values[variable_col] = values[variable_col] / len(
            self.wide_data[~self.wide_data["patientid"].isin(self.test_patientids)]
        )
    ...
    def _make_all_dynamic_features(self) -> None:
        ...
        if self.feature_processing_order == "alphabetical":
            list_of_all_data_sources = sorted(list_of_all_data_sources)

        if isinstance(self.feature_processing_order, list):
            list_of_all_data_sources = self.feature_processing_order
    ...
    def make_all_features(self) -> None:
        ...
        temp_features = self.static_features.copy()
        temp_features.extend(self.dynamic_features)
