...
# Load SDS codes, removing unclassified entries
sds_codes = (
...
sds_codes = sds_codes.sort_values("count").drop_duplicates("kode").drop(columns="count")
...
# Remove duplicates and filter out rows with missing identifiers
for key, df in procedure_data.items():
    df.drop_duplicates(inplace=True)
    df.reset_index(drop=True, inplace=True)
    procedure_data[key] = df.query("dw_ek_forloeb.notna() or dw_ek_kontakt.notna()")

# Remove unwanted columns and invalid rows
for key, df in procedure_data.items():
...
# Convert identifiers to numeric format
for key, df in procedure_data.items():
...
# Download and rename additional data sources
